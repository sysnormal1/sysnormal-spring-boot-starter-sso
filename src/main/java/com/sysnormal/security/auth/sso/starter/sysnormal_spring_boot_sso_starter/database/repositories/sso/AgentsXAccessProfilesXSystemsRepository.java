package com.sysnormal.security.auth.sso.starter.database.repositories.sso;

import com.sysnormal.security.auth.sso.starter.database.entities.sso.AgentXAccessProfileXSystem;
import org.springframework.stereotype.Repository;

/**
 * AgentXAccessProfileXSystem repository
 *
 * @author aalencarvz1
 * @version 1.0.0
 */
@Repository
public interface AgentsXAccessProfilesXSystemsRepository extends BaseSsoRepository<AgentXAccessProfileXSystem, Long> {


}

